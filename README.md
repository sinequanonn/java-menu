...
## 도메인 추출하기
- Coach(코치)
- 이름, 못 먹는 음식 리스트, 추천 받은 리스트
- Category (카테고리) - enum?
- 일식 - 1, 한식 - 2, 중식 - 3, 아시안 - 4, 양식 - 5
- MenuRepository
  - Map<Category, List<String>> menus;
- MenuRecommender
- 월 -> 금 이런 데이터는 상관X
- 5번 반복할 동안 그 상태를 어디에다가 저장할지만 생각하면 될 듯
- ResultRepository
  - List<Coach> coaches;
  - List<String> categories;

## 기능 구현 목록
- [x] 카테고리 데이터 및 메뉴 데이터를 저장한다.
- [x] 점심 메뉴 추천 시작 메시지를 화면에 출력한다.
  - `점심 메뉴 추천을 시작합니다.`
- [x] 사용자로부터 코치의 이름을 입력 받는다.
  - `코치의 이름을 입력해 주세요. (, 로 구분)`
  - 코치가 2~5명이 아닌 경우 예외를 발생시킨다.
- [x] 코치의 이름을 추출하여 코치를 저장한다.
    - 코치의 이름은 2~4글자 사이이다.
    - 이외의 경우는 예외를 발생시키고, 다시 입력 받는다.
- [x] 사용자로부터 각 코치가 못 먹는 메뉴를 입력 받는다.
- [x] 코치가 못 먹는 메뉴 유효성을 검증한다.
  - 존재하지 않는 메뉴인 경우 예외 발생 
  - 코치가 못 먹는 메뉴는 최소 0개, 최대 2개이다. (`,`로 구분해서 입력한다.)
  - 먹지 못하는 메뉴가 없으면 빈 값을 입력한다.

- ==추천 프로세스==
- [x] 1~5 사이 무작위 값을 뽑아내서 카테고리를 추출한다.
  - 한 주에 같은 카테고리는 최대 2회까지만 존재할 수 있다.
- [x] 카테고리에 따라 무작위 메뉴 한 개를 뽑아낸다.
  - [x] 한 주에 카테고리가 2개 이상인 지 검증한다.
- [x] 각 코치에게 한 주에 중복되지 않는 메뉴를 추천해야 한다.
  - 이미 코치에게 중복되는 메뉴가 있다면 다시 무작위 메뉴 한 개를 뽑아낸다.
- ==추천 프로세스 완료==
